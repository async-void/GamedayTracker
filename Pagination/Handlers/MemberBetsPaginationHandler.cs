...
{
    public class MemberBetsPaginationHandler(IDiscordEmbedService embedService): IPaginationHandler
    {
        public async Task HandleNextAsync(object data, InteractionCreatedEventArgs eventArgs)
        {
            var pagination = (MemberBetsPaginationData)data;

            pagination.CurrentPage++;

            var msg = await embedService.CreateMemberBetsPage(
                pagination.Bets,
                pagination.DiscordClient,
                pagination.CurrentPage);
